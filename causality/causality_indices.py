...
@author: tedinburgh
https://github.com/tedinburgh/causality-review/blob/main/causality-review-code/causality_indices.py
"""
...
def _transfer_entropy(x, y, mx=1, my=1, r=8, units='nats', \
    ...
    if effective:
        ...
        for s in range(shuffle):
            ...
            pts_xy_shuffled = pts_xy_shuffled[:, list(range(mx + my)) + [mx + my + 1]]
            pts_yx_shuffled = embedding(x, y[ind], mx = mx, my = my, h = 1)
            pts_yx_shuffled = pts_yx_shuffled[:, list(range(mx + my)) + [mx + my]]
...
def _transfer_entropy_ksg(x, y, mx=1, my=1, k=4, effective = False, \
        shuffle=10, seed=2212, metric='chebyshev'):
    ...
    # is xy_cols)
    def ksg_xy_algorithm(pts, x_cols, y_cols, hx_cols, k = k, metric = metric):
        ...
        n_xh = KDTree(pts[:, x_cols + hx_cols], metric = metric).query_radius(pts[:, x_cols + hx_cols], dists, count_only = True) - 1
        n_xy = KDTree(pts[:, x_cols + y_cols], metric = metric).query_radius(pts[:, x_cols + y_cols], dists, count_only = True) - 1
        n_x = KDTree(pts[:, x_cols], metric = metric).query_radius(pts[:, x_cols], dists, count_only = True) - 1

        # The transfer entropy via Krakov estimator (2004) is given in
        # Gomez-Herrero et al (2015) as:
        # TE_{Y->X} = digamma(k) - (digamma(n_x1) + digamma(n_xy) - digamma(n_x)) / n
        vfunc = np.vectorize(lambda z: digamma(max(z, 1)))
        te = digamma(k) - \
    ...
    # te_xy is the transfer entropy of x given y e.g. TE(X|Y)
    te_xy, n_x, n_xh, n_xy = ksg_xy_algorithm(pts, y_cols, x_cols, [mx + my + 1])
    te_yx, n_y, n_yh, n_yx = ksg_xy_algorithm(pts, x_cols, y_cols, [mx + my])
    return te_xy, te_yx
...
def _coarse_grained_transinformation_rate(x, y, k=4, tau_max=15, \
    ...
    def cmi_ksg(x, y, z, k = k, metric = metric):
        ...
        # Note the inequality is strict here! Hence the - 1e-16 below
        n_xz = KDTree(pts[:, [0, 2]], metric = metric).query_radius(pts[:, [0, 2]], dists, count_only = True) - 1
        n_yz = KDTree(pts[:, [1, 2]], metric = metric).query_radius(pts[:, [1, 2]], dists, count_only = True) - 1
        n_z = KDTree(pts[:, 2].reshape(-1, 1), metric = metric).query_radius(pts[:, 2].reshape(-1, 1), dists, count_only = True) - 1
...
def _extended_granger_causality(x, y, mx=2, my=2, L=5, delta=5, \
    ...
    for ll in range(L):
        ...
        var[ll, 0] = sm.OLS(pts_ll[:, mx + my], sm.add_constant(pts_ll[:, x_cols + y_cols])).fit().scale
        var[ll, 1] = sm.OLS(pts_ll[:, mx + my], sm.add_constant(pts_ll[:, x_cols])).fit().scale
        var[ll, 2] = sm.OLS(pts_ll[:, mx + my + 1], sm.add_constant(pts_ll[:, x_cols + y_cols])).fit().scale
        var[ll, 3] = sm.OLS(pts_ll[:, mx + my + 1], sm.add_constant(pts_ll[:, y_cols])).fit().scale
...
def predictability_improvement(X, mask, **args):
    if mask is None: mask = np.zeros((len(X),len(X))).astype(bool)
    mask_idx = np.where(~mask)
