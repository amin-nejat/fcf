...
from Causality.responseAnalysis import analyzeInterventions,causalityVsResponse
...
for dkInd,dk in enumerate(usableDataKeys):
     ...
     for afferentInd,afferent in enumerate(set(stim_chs)):
          ...
          analysisID=dk[1]+"_stimCh="+str(afferent+1)#+ shortPulse/longPulse
          print("responses analysis completed for "+analysisID)
          analysisIdStrings.append(analysisID) 
# %%
...
for resp_measure_name in resp_measure_names:
     ...
     for analysisInd in range(nAnalyses):
          ...
          causalityVsResponse(
                    ...
                    savingFilename=figuresFolder+analysisIdStrings[analysisInd]+"_respMeasure="+resp_measure_name,
                    corrMethod=log["corrMethod"]
                    )
